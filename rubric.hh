...
// rubric.hh
//
// Minimalist one-header unit test framework that organizes tests into
// a scored rubric, for classroom grading purposes.
//
///////////////////////////////////////////////////////////////////////////////
...
// Macro to signal that a test failed with the given message. line and
// file are automatically filled in by preprocessor directives.
#define TEST_FAIL(message) throw TestFailureException(__LINE__, __FILE__, std::string(message))

// A RubricCriterion is one line-item (row) in a rubric. It carries a
...
// Test macros. Each macro calls TEST_FAIL when the stated expectation
// is not met.

// Expects the expression (expr) to be false.
#define TEST_FALSE(message, expr) { if (expr) { TEST_FAIL(message); } }

// Expects the expression (expr) to be true.
#define TEST_TRUE(message, expr) TEST_FALSE(message, ! (expr) );

// Expects (x == y).
#define TEST_EQUAL(message, x, y) TEST_TRUE(message, (x) == (y) );
